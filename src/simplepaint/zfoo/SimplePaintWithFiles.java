...
// MTH: add timer to JPanel:
	...
	timer.setCoalesce(false); // This will coalesce pending firings - I'm not sure what it will do
...
//MTH
	...
	//if (!seis.timerOn) {  // Timer is off so draw everthing
	 ...
	 if (seis.timerOn) seis.iframe++;
         //if (seis.iframe == seis.Npts + 1) { // Stop the animation and return buttons to orig state

/*
         if (seis.iframe == seis.Npts) { // Stop the animation and return buttons to orig state
           timer.stop();
           play.setText("Play"); // Change button from Pause to Play
           //repaint();
        }
